...
**PATH note:** this command runs in a non-interactive shell that doesn't source the login profile, so `nix` and `darwin-rebuild` may not be on `PATH` (`nix: command not found`). Prefix every command below with `export PATH="/nix/var/nix/profiles/default/bin:$PATH" && …` so the Determinate Nix binaries resolve.

Steps:

1. **Build:** run `export PATH="/nix/var/nix/profiles/default/bin:$PATH" && nix build .#darwinConfigurations.mbp.system`.
...
2. **Switch** (skip if the argument was `build`): run `export PATH="/nix/var/nix/profiles/default/bin:$PATH" && sudo ./result/sw/bin/darwin-rebuild switch --flake .#mbp`.
   ...
   - On failure, surface the activation error plainly (activation failures are usually a launchd agent, a file collision, or a Homebrew step — say which). Note that a failed **Homebrew bundle** step (e.g. a stale `*.incomplete` download lock) does not roll back the nix/home-manager parts — user packages still install; say so rather than reporting a blanket failure.
