...
- **pounce daemon runs from a signed copy**: macOS keys an Accessibility (TCC) grant to a code-signing identity, but the store build is adhoc-signed (cdhash changes every rebuild). The `pounceDaemonLaunch` wrapper in `configuration.nix` copies `Pounce.app` to `~/.local/state/pounce/Pounce.app` and re-signs it with the Apple Development identity (stable designated requirement) before exec'ing the daemon — so the grant survives rebuilds. **Don't repoint the agent back at the store path.** One-time bootstrap after the first rebuild: run `~/.local/state/pounce/Pounce.app/Contents/MacOS/pounce --request-accessibility` and approve the prompt (and the keychain "Always Allow" dialog the first time `codesign` runs, so the partition list is set). Verify with `--check-accessibility` (prints `true`/`false`) and `codesign -d -r- ~/.local/state/pounce/Pounce.app` (the requirement string is what must stay constant across rebuilds).

## Patterns

- **New `pounce` command** (custom Swift command palette): add a `commands/<id>.sh`, register it in `pkgs/pounce-commands/default.nix` (`name` / `description` / SF Symbol `icon` / `script`). `pounce-palette` runs `pounce --launcher`, which natively enumerates installed apps (boosting freshly-installed ones) and merges them with these commands; apps are launched by the daemon itself. Utility sub-menus (ports/wifi/brew-services) call plain `pounce` and swap into the same live window without re-opening — don't reintroduce a close→reopen between steps.
