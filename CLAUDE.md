...
- **choose daemon runs from a signed copy**: macOS keys an Accessibility (TCC) grant to a code-signing identity, but the store build is adhoc-signed (cdhash changes every rebuild). The `chooseDaemonLaunch` wrapper in `configuration.nix` copies `Choose.app` to `~/.local/state/choose/Choose.app` and re-signs it with the Apple Development identity (stable designated requirement) before exec'ing the daemon — so the grant survives rebuilds. **Don't repoint the agent back at the store path.** One-time bootstrap after the first rebuild: run `~/.local/state/choose/Choose.app/Contents/MacOS/choose --request-accessibility` and approve the prompt (and the keychain "Always Allow" dialog the first time `codesign` runs, so the partition list is set). Verify with `--check-accessibility` (prints `true`/`false`) and `codesign -d -r- ~/.local/state/choose/Choose.app` (the requirement string is what must stay constant across rebuilds).
