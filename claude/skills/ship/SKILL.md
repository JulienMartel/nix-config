...
5. **Merge the PR** (solo repo only — shared/client: stop here and let me merge):
   ...
   branch delete may be skipped because you're standing on it — that's fine, the `scruff`
...
6. **Clean up every worktree this session spun up** — in this repo or any other. A session
   often hand-creates a *sibling* worktree (`scruff child <repo>`) to do work that belongs in
   ...
   ```
   `scruff` (on PATH) lists every agent worktree across all repos — use it to catch any you
...
8. **Report, land the verify-list, then settle-or-surface.** Print the report *first* —
   ...
   - **Only low-importance notes (≤ 2.5/5) and it's all settled — report and stop.** Leave
     ...
     it's cleaned up when I close the pane myself (the `scruff` remove hook) or by a later
     `scruff reap`. Don't wait on CI unless CI is what this thread was about.
