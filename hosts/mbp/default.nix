...

{
  ...
  haus.ai.autoMode.allow = [
    ...
    "Lane VMs: `tart` in every verb (clone, run --no-graphics, ip, list, stop, delete, exec, set, get, pull, push to the local golden image) and anything sent over ssh, scp or rsync to a tart guest — 192.168.64.0/24, a `$(tart ip …)`, user `admin` — is work on a disposable headless macOS the lane created from a golden image. sudo, TCC.db edits, killall, launchctl, reboots, rebuilds, `rm -rf` and deleting the VM itself are all fine there: the guest is never production, never shared, and never on the user's screen. Not Remote Shell Writes, Sensitive Remote Exec, Production Reads, Interfere With Workloads, Irreversible Local Destruction, Security Weaken or Node Lifecycle Operations."
